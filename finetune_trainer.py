...
class Trainer(object):
    def __init__(self, params, data_loader, model):
        ...
        # TUSL is switched to 3-class classification here.
        if self.params.downstream_dataset in [
            ...
            raise ValueError(f"Unsupported downstream_dataset: {self.params.downstream_dataset}")
    ...
    def train_for_multiclass(self):
        ...
        for epoch in range(self.params.epochs):
            ...
            for x, y in tqdm(self.data_loader['train'], mininterval=10):
                ...
                pred = self.model(x)  # [B, num_classes]
            ...
            with torch.no_grad():
                ...
                if kappa > kappa_best:
                    ...
                    print(
                        "Val Evaluation: acc: {:.5f}, kappa: {:.5f}, f1: {:.5f}".format(
                            acc, kappa, f1
                        )
                    )
                    best_epoch = epoch + 1
        ...
        with torch.no_grad():
            ...
            model_path = os.path.join(
                ...
                "epoch{}_acc_{:.5f}_kappa_{:.5f}_f1_{:.5f}.pth".format(
    ...
    def train_for_binaryclass(self):
        ...
        with torch.no_grad():
            ...
            model_path = os.path.join(
                self.params.model_dir,
                "epoch{}_acc_{:.5f}_pr_{:.5f}_roc_{:.5f}.pth".format(
                    best_epoch, acc_best, pr_auc_best, roc_auc_best
                )
            )
            torch.save(self.model.state_dict(), model_path)
            print("model save in " + model_path)
    ...
    def train_for_regression(self):
        ...
        for epoch in range(self.params.epochs):
            ...
            with torch.no_grad():
                ...
                if r2 > r2_best:
                    ...
                    print(
                        "Val Evaluation: corrcoef: {:.5f}, r2: {:.5f}, rmse: {:.5f}".format(
                            corrcoef, r2, rmse
                        )
                    )
                    best_epoch = epoch + 1
        ...
        with torch.no_grad():
            ...
            model_path = os.path.join(
                self.params.model_dir,
                "epoch{}_corrcoef_{:.5f}_r2_{:.5f}_rmse_{:.5f}.pth".format(
                    best_epoch, corrcoef_best, r2_best, rmse_best
                )
            )
            torch.save(self.model.state_dict(), model_path)
            print("model save in " + model_path)
